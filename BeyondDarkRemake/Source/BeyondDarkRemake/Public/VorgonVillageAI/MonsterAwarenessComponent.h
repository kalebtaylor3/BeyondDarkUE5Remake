...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Awareness|Config|Sight")
	...
	// Distance exponent for sight gain (>1 = close-up ramps much faster than mid-range)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Awareness|Config|Sight")
	float SightDistanceExponent = 1.6f;

	// Within this distance, Vorgon basically reaches full awareness almost instantly
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Awareness|Config|Sight")
	float InstantFullAwarenessDistance = 200.f;

	// Base gain when we hear a noise (scaled by loudness)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Awareness|Config|Hearing")
	float HearingBaseGain = 15.f;

	// Awareness decay per second when calm (no recent stimuli)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Awareness|Config|Decay")
	...
	UFUNCTION(BlueprintCallable, Category = "Awareness")
	void ForceState(EMonsterState NewState, bool bResetEngagement = true);
...
private:
	...
	void ApplyMovementForState(EMonsterState NewState);
